...
def create_lfg():
    ...
    if request.method == 'POST':
        ...
        if not game or not description or not level_required:
            flash("Please fill out all required fields.")
            return redirect(url_for('create_lfg'))
        ...
        flash("LFG post created successfully!")
        return redirect(url_for('home'))
...
def edit_lfg(id):
    ...
    if isinstance(post_owner, ObjectId):
        post_owner = str(post_owner)

    if not is_admin() and str(post_owner) != session['user']['id']:
...
def post_detail(id):
    ...
    created_by = lfg_post.get('created_by')
    
    user = None
    try:
        user = db.users.find_one({'_id': ObjectId(created_by)})
    except (InvalidId, TypeError):
        user = {'username': created_by}  # Fallback for non-ObjectId values
