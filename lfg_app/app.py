...
def create_lfg():
    ...
    if request.method == 'POST':
        ...
        db.lfg.insert_one({
            ...
            'region': region,
            'created_by': str(session['user']['id']),  # Ensure ID is stored as a string
            'created_at': datetime.now(timezone.utc),
            'contact': session['user'].get('email', ''),  # Store contact info
            'role': session['user'].get('role', 'user')  # Default role to 'user'
        })
